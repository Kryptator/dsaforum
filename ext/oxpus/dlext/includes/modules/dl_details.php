...
*/
$dl_files = array();
$dl_files = \oxpus\dlext\includes\classes\ dl_files::all_files(0, '', 'ASC', '', $df_id, $modcp, '*');
...
if (($cat_auth['auth_mod'] || ($this->auth->acl_get('a_') && $this->user->data['is_registered'])) && !\oxpus\dlext\includes\classes\ dl_auth::user_banned())
{
	$modcp = ($modcp) ? 1 : 0;
...
$check_status = array();
$check_status = \oxpus\dlext\includes\classes\ dl_status::status($df_id, $this->helper);
...
* User is banned?
*/
if (\oxpus\dlext\includes\classes\ dl_auth::user_banned())
{
	$s_dl_userban = true;
}
else
...
if ($index[$cat_id]['comments'] && \oxpus\dlext\includes\classes\ dl_auth::cat_auth_comment_read($cat_id))
{
	/*
	...
	*/
	...
	if (!\oxpus\dlext\includes\classes\ dl_auth::cat_auth_comment_post($cat_id))
	{
		$allow_manage = false;
		$deny_post = true;
	}
	...
	if ($action == 'save' && !$deny_post && $comment_text)
	{
		...
		if ($index[$cat_id]['approve_comments'] || \oxpus\dlext\includes\classes\ dl_auth::user_admin())
		{
			$approve = true;
		}
		else
		...
		if ($approve)
		{
			$processing_user = (\oxpus\dlext\includes\classes\ dl_auth::cat_auth_comment_read($cat_id) == 3) ? 0 : \oxpus\dlext\includes\classes\ dl_auth::dl_auth_users($cat_id, 'auth_mod');
			...
			\oxpus\dlext\includes\classes\ dl_email::send_comment_notify($mail_data, $this->helper, $ext_path);
...
{
	$file_size_out = \oxpus\dlext\includes\classes\ dl_format::dl_size($dl_files['file_size'], 2);
}
else
{
...
$s_edit_button = false;
$s_edit_thumbs = false;
if (!$this->user->data['is_bot'] && \oxpus\dlext\includes\classes\ dl_auth::user_auth($dl_files['cat'], 'auth_mod') || ($this->config['dl_edit_own_downloads'] && $dl_files['add_user'] == $this->user->data['user_id']))
{
	$s_edit_button = true;
...
$this->template->assign_vars(array(
	...
	'RATING_IMG'			=> \oxpus\dlext\includes\classes\ dl_format::rating_img($rating_points, $s_rating_perm, $df_id),
...
$cp = new \oxpus\dlext\includes\helpers\ custom_profile();
