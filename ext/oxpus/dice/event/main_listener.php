...
class main_listener implements EventSubscriberInterface
{
	...
	*/
	...
	public function format_dice_rolls($event)
	{
		...
		if ($bbcode_uid)
		{
			$message = preg_replace_callback("#\[dice:$bbcode_uid\](.+?)\[/dice:$bbcode_uid\]#is", function($match) { return $this->_decodedice($match[1], ''); }, $message);
			$message = preg_replace_callback("#\[dice=(.+?):$bbcode_uid\](.*?)(([0-9]+:)?[0-9]+)(.*?)\[/dice:$bbcode_uid\]#is", function($match) { return $this->_decodedice($match[3], $match[1]); }, $message);
		}
